...
today = time.strftime("%Y{y}%m{m}%d{d}",time.localtime()).format(y='年',m='月',d='日')  # 可以更具需求任意修改中间的内容，带式不能显示中文，通过format显示中文

testcase = 'excel_to_word\\testcase\\华新收款测试用例.xlsx'
save_file = 'excel_to_word\\testcase\\%s华新收款测试用例.docx' %today


class Exchange:
    '''实现将测试用例excel根据绘制的模板转为word'''
    ...
    def run():
        ...
        xlsx = xlrd.open_workbook(testcase)
